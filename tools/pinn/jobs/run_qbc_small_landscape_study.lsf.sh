#!/usr/bin/env bash
#BSUB -J pinn_qbc_landscape_study
#BSUB -q gpua100
#BSUB -n 1
#BSUB -W 24:00
#BSUB -R "rusage[mem=16GB]"
#BSUB -oo outputs/lsf_logs/pinn_qbc_landscape_study.%J.out
#BSUB -eo outputs/lsf_logs/pinn_qbc_landscape_study.%J.err
...
if ! "${PYTHON_BIN}" -c "import omegaconf" >/dev/null 2>&1; then
  ...
  echo "[ERROR] submit with PYTHON_BIN=/path/to/venv/bin/python bsub < tools/pinn/jobs/run_qbc_small_landscape_study.lsf.sh"
  exit 1
fi
...
EXPERIMENT_TAG="${EXPERIMENT_TAG:-qbc_b1024_adam300_landscape_${STAMP}}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/outputs/pinn_hpc_experiments/${EXPERIMENT_TAG}}"
EXPORT_ROOT="${EXPORT_ROOT:-${REPO_ROOT}/results/pinn_landscape/${EXPERIMENT_TAG}}"
...
CHECKPOINT_FRACTIONS="${CHECKPOINT_FRACTIONS:-0.5}"
LANDSCAPE_CHECKPOINT_TAGS="${LANDSCAPE_CHECKPOINT_TAGS:-init,epoch_050pct,last}"
...
CMD=(
  "${PYTHON_BIN}"
  tools/pinn/run_qbc_small_landscape_study.py
  ...
  --landscape-checkpoint-tags "${LANDSCAPE_CHECKPOINT_TAGS}"
  --grid "${GRID_MODE}"
...
echo "[pinn-qbc-landscape-study] output_root=${OUTPUT_ROOT}"
echo "[pinn-qbc-landscape-study] export_root=${EXPORT_ROOT}"
echo "[pinn-qbc-landscape-study] command=${CMD[*]}"
