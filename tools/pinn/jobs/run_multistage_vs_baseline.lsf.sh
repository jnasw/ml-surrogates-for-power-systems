...
MULTISTAGE_RESIDUAL_PHASE_SEQUENCE="${MULTISTAGE_RESIDUAL_PHASE_SEQUENCE:-Adam:300}"
...
CMD=(
  ...
  --modes "${MODES}"
  --multistage-residual-phase-sequence "${MULTISTAGE_RESIDUAL_PHASE_SEQUENCE}"
...
echo "[multistage-vs-baseline] multistage_residual_phase_sequence=${MULTISTAGE_RESIDUAL_PHASE_SEQUENCE}"
if [[ -n "${PRESET}" ]]; then
