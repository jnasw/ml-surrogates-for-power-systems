...
def _active_init_weights_or_none(
    *,
    vrba_config: Any,
...
def _sample_supervised_rows_with_indices(
    ...
    *,
    ...
    if curriculum_enabled:
        ...
        active_max_bin = _curriculum_active_max_bin(
            config=config,
    ...
    if curriculum_enabled and bool(cfg_get(config, "pinn.curriculum.balance_active_bins", True)):
        sampled_local = _sample_curriculum_indices_balanced(
...
def _curriculum_active_max_bin(*, config: Any, global_epoch: int, num_bins: int) -> int:
    unlock_epochs = cfg_get(config, "pinn.curriculum.unlock_epochs", None)
    if unlock_epochs in (None, "null"):
        stage_length = int(cfg_get(config, "pinn.curriculum.stage_length_epochs", 100))
        if stage_length <= 0:
...
def _sample_curriculum_indices_balanced(
    *,
    ...
    if int(active_bins.shape[0]) <= 1:
        ...
        return eligible_indices.index_select(0, torch.as_tensor(local_positions, device=eligible_indices.device, dtype=torch.long))
    ...
    for idx, bin_id in enumerate(active_bins.tolist()):
        ...
        if desired >= int(bin_indices.shape[0]):
            ...
            selected = bin_indices.index_select(0, torch.as_tensor(choice, device=bin_indices.device, dtype=torch.long))
    ...
    *,
    ...
    def batch_iterator():
        for step_idx in range(steps_per_epoch):
            ...
            x_init_weights = None if dataset.train_init_weights is None else _sample_rows_with_indices(dataset.train_init_weights, init_idx)
            yield PinnBatch(
                x_data=x_data,
                y_data=y_data,
                x_data_dt_weights=x_data_dt_weights,
    ...
    for stage_idx in range(max_stages):
        ...
        if _multistage_loss_ref_enabled(config):
            stage_ref_losses, _ = _measure_pinn_state(
                ...
                x_init_weights=_active_init_weights_or_none(vrba_config=vrba_config, x_init_weights=dataset.train_init_weights),
        ...
        for phase in stage_optimizer_phases:
            ...
            for epoch in range(1, phase.epochs + 1):
                ...
                epoch_train_x, epoch_train_y, epoch_train_data_idx = _sample_supervised_rows_with_indices(
                    dataset.train_x,
                    dataset.train_y,
                    config,
                ...
                epoch_train_init_weights = epoch_pool_batch.x_init_weights
                active_init_weights = _active_init_weights_or_none(
                    vrba_config=vrba_config,
                    x_init_weights=epoch_train_init_weights,
