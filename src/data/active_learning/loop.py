...
from src.data.active_learning.acquisition import acquire_diverse, acquire_topk, select_qbc_marker_hybrid
from src.data.active_learning.disagreement import score_disagreement
from src.data.active_learning.ensemble import DeepEnsemble, train_ensemble
from src.data.active_learning.marker_utils import (
    MarkerFeatureConfig,
...
class QBCConfig:
    ...
    def from_runtime(
        ...
        *,
        ...
        candidate_method = str(cfg_get(config, "active.candidate_method", "sobol"))
        diversify = bool(cfg_get(config, "active.diversify", True))
        batch_size = int(cfg_get(config, "active.predict_batch_size", 2048))
        disagreement_metric = str(cfg_get(config, "active.disagreement.metric", "variance_mean"))
        diversity_preselect_factor = int(cfg_get(config, "active.diversity.preselect_factor", 5))
        diversity_uncertainty_weight = float(cfg_get(config, "active.diversity.uncertainty_weight", 0.7))
        diversity_distance_weight = float(cfg_get(config, "active.diversity.distance_weight", 0.3))
        diversity_norm_uncertainty = bool(cfg_get(config, "active.diversity.normalize_uncertainty", True))
        diversity_norm_distance = bool(cfg_get(config, "active.diversity.normalize_distance", True))
        log_every = int(cfg_get(config, "active.log_every", 5))
        acquisition_strategy = str(cfg_get(config, "active.acquisition_strategy", "qbc_only")).lower()
...
def run_marker_loop(
    ...
    candidate_method = str(cfg_get(config, "active.candidate_method", "lhs"))

    explained = float(cfg_get(config, "active.marker.pca_explained_variance", 0.90))
    w_div = float(cfg_get(config, "active.marker.weights.diversity", 0.6))
    w_sparse = float(cfg_get(config, "active.marker.weights.sparsity", 0.4))
    preselect_factor = int(cfg_get(config, "active.marker.preselect_factor", 5))
    alpha_score = float(cfg_get(config, "active.marker.greedy_score_weight", 0.7))
    k_density = int(cfg_get(config, "active.marker.k_density", 15))
    settling_fraction = float(cfg_get(config, "active.marker.settling_fraction", 0.05))
    include_anchor = bool(cfg_get(config, "active.marker.include_anchor_state_markers", True))
    eval_members = int(cfg_get(config, "active.marker.eval_members", int(getattr(config, "qbc_M", 3))))
    eval_every_round = bool(cfg_get(config, "active.marker.eval_every_round", False))
    eval_batch_size = int(cfg_get(config, "active.predict_batch_size", 2048))
    log_every = int(cfg_get(config, "active.log_every", 5))
